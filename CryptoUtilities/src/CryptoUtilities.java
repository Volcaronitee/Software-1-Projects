...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        if (!m.isZero()) {
            NaturalNumber copy = new NaturalNumber2(m);
            NaturalNumber remain = n.divide(copy);
            n.copyFrom(copy);
            m.copyFrom(remain);
            reduceToGCD(n, m);
        }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        NaturalNumber copy = new NaturalNumber2(p);
        NaturalNumber remain = copy.divide(new NaturalNumber2(2));

        if (p.isZero()) {
            n.copyFrom(new NaturalNumber2(1));
        } else if (!p.equals(new NaturalNumber2(1))) {
            if (remain.isZero()) {
                n.multiply(num);
                remain = n.divide(m);
                n.copyFrom(remain);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber check1 = new NaturalNumber2(w);
        powerMod(check1, two, n);

        // w ^ (n-1) mod n
        NaturalNumber check2 = new NaturalNumber2(w);
        NaturalNumber copy = new NaturalNumber2(n);
        copy.decrement();
        powerMod(check2, copy, n);

        if (check1.equals(one)) {
            composite = true;
        } else if (!check2.equals(one)) {
            composite = true;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        for (int i = 0; i < size && prime; i++) {
            ...
            if (!(new NaturalNumber2(1).compareTo(rand) < 0)) {
                rand.add(two);
            }
